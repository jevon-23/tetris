...
block *move_line_block(game_board *board, line_block *l, int row, int col) {
  if (row > l->row_pos) {
    ...
    for (int i = 0; i < l->dim.cols; i++) {
      block *lower_block =
          get_block(board, l->row_pos + l->dim.rows, l->col_pos + i);
      if (lower_block == NULL)
        continue;
  ...
  line_block *new_block = make_line_block();
...
bool place_vertical_line_block(game_board *board, line_block *l, int row,
                               int col) {
  for (int i = 0; i < l->dim.rows; i++) {
...
bool place_horizontal_line_block(game_board *board, line_block *l, int row,
                                 int col) {
  for (int i = 0; i < l->dim.cols; i++) {
    /* Place the value on the board */
